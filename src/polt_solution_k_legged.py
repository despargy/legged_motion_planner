...
def store_onecol_data(data, step, traj_size):
    ...
    for pp in range(traj_size):
        ...
        F1_x.append(data.iloc[pp*step + 9, 0])
        F1_y.append(data.iloc[pp*step + 10, 0])
        F1_z.append(data.iloc[pp*step + 11, 0])

        S1_x.append(data.iloc[pp*step + 12, 0])
        S1_y.append(data.iloc[pp*step + 13, 0])
        S1_z.append(data.iloc[pp*step + 14, 0])

    list_com_solution = [CoM_x, CoM_y, CoM_z]
    list_lin_solution = [lin_x, lin_y, lin_z]
    list_ang_solution = [ang_x, ang_y, ang_z]
    list_forces_solution = [F1_x, F1_y, F1_z]
    list_foot_pos_solution = [S1_x, S1_y, S1_z]
...
if __name__ == '__main__':
    ...
    # desired
    step = 21
    data_desired = pd.read_csv('../records/DesiredTrajectory50_100.csv', header=None)  # It has no header
    traj_size = data_desired.shape[0]-1
    ...
    # for leg_item in range(1):
    plt.plot(list_foot_pos_solution[0], label="x solution")
    plt.plot(list_foot_pos_solution[1], label="y solution")
    plt.plot(list_foot_pos_solution[2], label="z solution")

    plt.plot(list_foot_pos_desired[1][0], label="x desired")
    plt.plot(list_foot_pos_desired[1][1], label="y desired")
    plt.plot(list_foot_pos_desired[1][2], label="z desired")
    plt.legend()
