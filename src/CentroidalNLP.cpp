...
bool CentroidalNLP::init_before_start()
{
  ...
  // init indexs to help you map x variables with CoM, l, k, Forces, Steps
  ...
  // indexs to help number-map the constraint functions
  index_conF_eq_1 = 0;
  ...
  // total number of constraint functions fow each dt
  g_per_dt = index_conF_eq_7 + 3; // g_per_dt has to be 17 in case of 2 legs

  // indexs to help number-map the non values in constraint functions
...
)
{
  // n_points = n_points - 1 ;
   ...
   n = n_per_dt*n_points; // n_points is without the init position
   ...
   nnz_jac_g = nnz_jac_g_per_dt*n_points;

   std::cout<<"nnz_jac_g_per_dt = "<< nnz_jac_g_per_dt<<"\n"<<std::endl;

   std::cout<<"nnz_jac_g = "<< nnz_jac_g<<"\n"<<std::endl;

   // do not consider the hessian of the lagrangian
   nnz_h_lag = 0;
...
)
{
  ...
  for(int pp=0; pp<n_points; pp++)
  {
    ...
    {
      x_l[pp*n_per_dt + index_Forces + 3*ll + 0] = -1000; x_u[pp*n_per_dt + index_Forces + 3*ll + 0] = 1000;
      x_l[pp*n_per_dt + index_Forces + 3*ll + 1] = -1000; x_u[pp*n_per_dt + index_Forces + 3*ll + 1] = 1000;
      x_l[pp*n_per_dt + index_Forces + 3*ll + 2] = 0; x_u[pp*n_per_dt + index_Forces + 3*ll + 2] = 3000;
    }
  }
  ...
  // std::cout<<"here get_starting_point"<<std::endl;
   ...
   for(int ll=0; ll < p_agent->k_legs; ll++) //k_legs //TODO
   {
    ...
    //multi 3* since each legs has x y z
     ...
     // g < 0 so add - in front
     x[index_Forces + 3*ll + 2] = abs(p_agent->mass * p_agent->g) / p_agent->k_legs; // z
   }

   // Steps, for each leg
   for(int ll=0; ll < p_agent->k_legs; ll++) //k_legs
   {
     // Init Foot Step Position
  ...
  // std::cout<<"here eval_grad_f"<<std::endl;
   ...
   for(int pp =0; pp < n_points; pp++)
   {
     // std::cout<<"pp*g_per_dt = "<<pp*n_per_dt<<"\n"<<std::endl;
      ...
      // use desired + 1 since the 0 is considered as init point, so we see the next i each time
...
)
{
  // std::cout<<"here eval_g"<<std::endl;
   ...
   for(int pp =0; pp < n_points; pp++)
   {
     ...
     {
       // now i try aybz - azby
       sum_cross_x += x[(pp)*n_per_dt + index_CoM_pos + 0] - abs( x[(pp)*n_per_dt + index_Step + 3*ll + 2]*x[(pp)*n_per_dt + index_Forces + 3*ll + 1]
                                                                                - x[(pp)*n_per_dt + index_Step + 3*ll + 1 ]*x[(pp)*n_per_dt + index_Forces + 3*ll + 2] );
       // now i try - abs( axbz - azbx)
       sum_cross_y += x[(pp)*n_per_dt + index_CoM_pos + 1] + abs( x[(pp)*n_per_dt + index_Step + 3*ll + 2]*x[(pp)*n_per_dt + index_Forces + 3*ll + 0]
                                                                                -  x[(pp)*n_per_dt + index_Step + 3*ll + 0]*x[(pp)*n_per_dt + index_Forces + 3*ll + 2]);
       // now i try axby - aybx
       sum_cross_z += x[(pp)*n_per_dt + index_CoM_pos + 2] - abs( x[(pp)*n_per_dt + index_Step + 3*ll + 1]*x[(pp)*n_per_dt + index_Forces + 3*ll + 0]
                                                                                - x[(pp)*n_per_dt + index_Step + 3*ll + 0]*x[(pp)*n_per_dt + index_Forces + 3*ll + 1] );
     }
...
)
{
  // std::cout<<"here eval_jac_g"<<std::endl;
      ...
      if (values == NULL) {
        ...
        for(int pp =0; pp < n_points; pp++)
        {
          ...
          for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
          {
            iRow[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 0] = pp*g_per_dt + index_conF_eq_6 + 0; jCol[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 0] = pp*n_per_dt + index_Forces + 3*inner_ll + 1; // has /dfi_y
            iRow[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 1] = pp*g_per_dt + index_conF_eq_6 + 0; jCol[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 1] = pp*n_per_dt + index_Forces + 3*inner_ll + 2; // has /dfi_z

          }

          for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
          {
            iRow[pp*nnz_jac_g_per_dt + index_partial_eq_6 + (2+2*p_agent->k_legs) + 2*inner_ll + 0] = pp*g_per_dt + index_conF_eq_6 + 0; jCol[pp*nnz_jac_g_per_dt + index_partial_eq_6 + (2+2*p_agent->k_legs) + 2*inner_ll + 0] = pp*n_per_dt + index_Step + 3*inner_ll + 1; // has /dsi_y
          ...
          for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
          {
            ...
            iRow[pp*nnz_jac_g_per_dt + index_partial_eq_6 + (2 + p_agent->k_legs*4) + 2 + 2*inner_ll + 1] = pp*g_per_dt + index_conF_eq_6 + 1; jCol[pp*nnz_jac_g_per_dt + index_partial_eq_6 + (2 + p_agent->k_legs*4) + 2 + 2*inner_ll + 1] = pp*n_per_dt + index_Forces + 3*inner_ll + 2; // has /dfi_z
          ...
          for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
          {
            ...
            iRow[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2*(2 + p_agent->k_legs*4) + (2+2*p_agent->k_legs) + 2*inner_ll + 1] = pp*g_per_dt + index_conF_eq_6 + 2; jCol[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2*(2 + p_agent->k_legs*4) + (2+2*p_agent->k_legs) + 2*inner_ll + 1] = pp*n_per_dt + index_Step + 3*inner_ll + 1; // has /dsi_y
  ...
  ////////////////////
            ...
            for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
            {
              if( x[(pp)*n_per_dt + index_Step + 3*inner_ll + 2]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 1]
                                                                                       - x[(pp)*n_per_dt + index_Step + 3*inner_ll + 1 ]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 2] > 0)
              {
                values[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 0] = +x[pp*n_per_dt + index_Step + 3*inner_ll + 2]*dt; //dfi_y
                values[pp*nnz_jac_g_per_dt + index_partial_eq_6 + 2 + 2*inner_ll + 1] = -x[pp*n_per_dt + index_Step + 3*inner_ll + 1]*dt; //dfi_z

                values[pp*nnz_jac_g_per_dt + index_partial_eq_6 + (2+2*p_agent->k_legs) + 2*inner_ll + 0] = -x[pp*n_per_dt + index_Forces + 3*inner_ll + 2]*dt; //dsi_y
            ...
            for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
            {
              if ( x[(pp)*n_per_dt + index_Step + 3*inner_ll + 2]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 0]
                      -  x[(pp)*n_per_dt + index_Step + 3*inner_ll + 0]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 2]>0)
              {
            ...
            for(int inner_ll=0; inner_ll<p_agent->k_legs; inner_ll++)
            {
              if( x[(pp)*n_per_dt + index_Step + 3*inner_ll + 1]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 0]
                         - x[(pp)*n_per_dt + index_Step + 3*inner_ll + 0]*x[(pp)*n_per_dt + index_Forces + 3*inner_ll + 1] >0)
              {
...
)
{
   ...
   std::ofstream myfile;
   myfile.open("/home/despargy/master_g15_ws/src/legged_motion_planner/results/solution.csv");
   for( Index i = 0; i < n; i++ )
   {
     myfile <<  x[i] <<"\n";
   }
